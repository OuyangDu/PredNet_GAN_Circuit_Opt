# this is almost identical to prednet_to_shift_sqaure_keep_center, but some parameters are different
import os
import numpy as np
...
def get_output(
    ...
    edge_ori, size_list, video_batch = rsg.generate_size_square_video_list(edge_ori=edge_ori, beta=beta, size_list=size_list, n_frames=n_frames, pixel_format=pixel_format)
...
output_path = os.path.join(DATA_DIR, head + '_square_dark_prednet_{}_{}_{}.hkl'.format(output_mode, neural_id, gamma))
hkl.dump(output_dark_square_center, output_path)
output_path = os.path.join(DATA_DIR, head + '_square_light_prednet_{}_{}_{}.hkl'.format(output_mode, neural_id, gamma))
hkl.dump(output_light_square_center, output_path)

label = {'output_mode': output_mode, 'neural_id': neural_id, 'edge_dir': edge_ori, 'size': size_list}
label_path = os.path.join(DATA_DIR, head + '_square_label_{}_{}_{}.hkl'.format(output_mode, neural_id, gamma))
