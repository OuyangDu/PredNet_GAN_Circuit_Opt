...
class PredNet(Recurrent):
    ...
    # Arguments
        ...
        mask_r_indices: list of lists of indices for which R units to mask in each layer. For example [[], [[0, 0, 0], [0, 1, 2,]], [], []]. This will silent R1's channel 0's 0, 0 and 1, 2 cells
    ...
    def __init__(self, stack_sizes, R_stack_sizes,
                 ...
                 LSTM_activation='relu', LSTM_inner_activation='hard_sigmoid',
                 output_mode='error', extrap_start_time=None,
                 data_format=K.image_data_format(), stddev=0.1, turn_noise=False,
                 r_mask_indices=None,
                 e_mask_indices=None,
                 **kwargs):
        ...
        self.r_mask_indices = r_mask_indices if r_mask_indices is not None else [[]] * self.nb_layers
        self.e_mask_indices = e_mask_indices if e_mask_indices is not None else [[]] * self.nb_layers
    ...
    def build(self, input_shape):
        self.noise = tf.keras.layers.GaussianNoise(self.stddev)
    ...
    def step(self, a, states):
        ...
        r_mask = [] # masking for R units
        e_mask = []
        for l in range(self.nb_layers):
            r_shape = K.shape(r_tm1[l])
            r_mask.append(create_batch_mask(r_shape, self.r_mask_indices[l]))
            e_shape = K.shape(e_tm1[l])
            e_mask.append(create_batch_mask(e_shape, self.e_mask_indices[l]))
        ...
        for l in reversed(range(self.nb_layers)):
            ...
            _c = self.noise.call(_c, training=self.turn_noise)
        ...
        for l in range(self.nb_layers):
            ...
            # compute errors
            #e_up = self.noise.call(ahat - a, training=self.turn_noise) # uncomment to add noise to e module
            e_up = ahat - a
            e_up = self.error_activation(e_up)
            e_down = a - ahat
            #e_down = self.noise.call(a - ahat, training=self.turn_noise)
            e_down = self.error_activation(a - ahat)

            _e = K.concatenate((e_up, e_down), axis=self.channel_axis) * e_mask[l]
            e.append(_e)
...
def create_batch_mask(r_shape, indices):
    """
    Create a mask for the tensor with shape `r_shape` where
    all elements at specified indices [batch, i, j, k] are set to 0.
    
    Parameters:
    r_shape (Tensor): A tensor representing the shape of the target tensor.
    indices (list of list of int): Indices to be masked in the format [[i, j, k], [i, j, k], ...].

    Returns:
    Tensor: A mask tensor of the same shape as the target tensor.
    """
    if len(indices) == 0:
        return tf.ones(r_shape, dtype=tf.float32)
    # Creating a mask for one sample with multiple indices
    single_sample_shape = [r_shape[1], r_shape[2], r_shape[3]]
    single_sample_updates = [1] * len(indices)
    single_sample_indices = indices
    single_sample_mask = tf.scatter_nd(single_sample_indices, single_sample_updates, single_sample_shape)
    single_sample_mask = tf.cast(tf.logical_not(tf.cast(single_sample_mask, tf.bool)), tf.float32)
    # Expanding the mask to the full batch size
    mask = K.expand_dims(single_sample_mask, axis=0)
    mask = K.tile(mask, [r_shape[0], 1, 1, 1])
    return mask

def all_indices(shape):
    """
    Generate all possible indices for a given shape.

    Parameters:
    shape (Tensor): A tensor representing the shape of a target tensor.

    Returns:
    list of list of int: A list containing all possible indices for the given shape.
    """
    # Extract the spatial dimensions, ignoring the batch dimension
    spatial_dimensions = [shape[i] for i in range(1, len(shape))]
    # Create an iterable of all possible index combinations
    all_idx = list(itertools.product(*[range(dim) for dim in spatial_dimensions]))
    return all_idx
